...
public class RealisasiSTPManager {
	
	public static String getRealisasiSTP(Date tanggal) {
		ConnectorUtil cu = new ConnectorUtil();
		Session session = cu.getSessionFactory().openSession();
		
		String sql = "select sum(jmlStpYgDibayar) from PembayaranSTP where " +
				"tglPembayaranStp between to_date('01/01/" + (tanggal.getYear() + 1900) + 
				"','DD/MM/YYYY') and to_date('" + tanggal.getDate() + "/" + 
				(tanggal.getMonth() + 1) + "/" + (tanggal.getYear() + 1900) + "','DD/MM/YYYY')";
		try {
