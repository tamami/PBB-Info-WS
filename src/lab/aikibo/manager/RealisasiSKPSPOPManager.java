...
public class RealisasiSKPSPOPManager {
	
	public static String getRealisasiSKPSPOP(Date tanggal) {
		ConnectorUtil cu = new ConnectorUtil();
		Session session = cu.getSessionFactory().openSession();
		
		String sql = "select sum(jmlSkpSpopYgDibayar) from PembayaranSKPSPOP where " +
				"tglPembayaranSkpSpop between to_date('01/01/" + (tanggal.getYear() + 1900) + 
